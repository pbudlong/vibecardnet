...
export async function registerRoutes(
  ...
  app.post('/api/demo/test-transaction', async (req, res) => {
    try {
      ...
      const totalAmount = 2.00;
      const splits = [
        { recipient: users[0].address, recipientWalletId: users[0].id, amount: '1.00', role: 'creator' },
        { recipient: users[1]?.address || users[0].address, recipientWalletId: users[1]?.id || users[0].id, amount: '0.70', role: 'upstream' },
        { recipient: users[2]?.address || users[0].address, recipientWalletId: users[2]?.id || users[0].id, amount: '0.30', role: 'actor' },
