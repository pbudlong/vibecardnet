...
const testTransactionLogs = [
  ...
  { time: "00:00:05", type: "info", message: "x402 atomic split: Creator 60% | Sharer 25% | Platform 15%" },
  { time: "00:00:06", type: "success", message: "Payment batch queued: 3 recipients" },
      ...
      >
        <div className="flex gap-4">
          ...
          <Card className="flex-1 p-3 bg-zinc-950 border-zinc-800" data-testid="card-terminal">
            ...
            <div className="h-48 overflow-y-auto font-mono text-[10px] space-y-1" data-testid="logs-container">
              {logs.map((log, i) => (
                <motion.div
                  key={i}
          ...
          <div className="flex flex-col gap-2">
            <Card className="p-3" data-testid="card-transaction-flow">
              ...
              <div className="space-y-2 text-center">
                <div className="p-2 rounded bg-emerald-500/10 border border-emerald-500/30" data-testid="flow-reward-pool">
                  <span className="text-[9px] text-emerald-400 font-medium">Reward Pool</span>
                  <p className="text-sm font-bold text-foreground">$500.00</p>
                </div>
                <ArrowDown className="h-4 w-4 mx-auto text-muted-foreground" />
                <div className="p-2 rounded bg-sky-400/10 border border-sky-400/30" data-testid="flow-x402-split">
                  <span className="text-[9px] text-sky-400 font-medium">x402 Split</span>
                  <p className="text-[10px] text-muted-foreground">Atomic Multi-Party</p>
                </div>
                <ArrowDown className="h-4 w-4 mx-auto text-muted-foreground" />
                <div className="p-2 rounded bg-zinc-500/10 border border-zinc-500/30" data-testid="flow-user-wallets">
                  <span className="text-[9px] text-zinc-400 font-medium">User Wallets</span>
                  <p className="text-[10px] text-muted-foreground">Circle Payouts</p>
                </div>
              </div>
            </Card>
          </div>
        ...
        >
          <Card className="p-3" data-testid="card-wallet-payouts">
              ...
              {walletPayouts.map((payout, i) => (
                ...
                >
                  <div className={`h-10 w-10 rounded-full flex items-center justify-center ${
                    ...
                    <Wallet className={`h-5 w-5 ${showPayouts ? 'text-sky-400' : 'text-zinc-600'}`} />
                  </div>
                  <span className="text-[10px] font-medium text-foreground">{payout.label}</span>
                  <span className={`text-sm font-bold ${showPayouts ? 'text-emerald-400' : 'text-zinc-600'}`} data-testid={`text-payout-${payout.label.toLowerCase()}`}>
                    {payout.amount}
                  </span>
                  <span className="text-[8px] text-muted-foreground font-mono">{payout.address}</span>
                </motion.div>
              ))}
            </div>
          </Card>
